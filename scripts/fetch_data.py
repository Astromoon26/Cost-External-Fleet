...
Stores flat aggregated rows per (year, month, owner, origin, pulau, area)
so the frontend can filter by any combination of dimensions.
"""
...
ORIGIN_NORM = {
    "cikarang":"Jababeka","jababeka":"Jababeka","jbbk":"Jababeka",
    "cikupa & jababeka":"Cikupa & Jababeka","cikupa":"Cikupa","sidoarjo":"Sidoarjo",
}

PULAU_NORM = {"papura":"Papua","bali nusra":"Bali Nusra"}

def parse_month(val):
    parts = re.split(r"[\s\-/]+", str(val).strip())
    return MONTH_ABBR.get(parts[0].lower()) if parts else None

def normalize_origin(val, site=""):
    s = str(val).strip()
...
def load_tabs(xl, tabs, year):
    ...
    for tab in tabs:
        ...
        if "Origin" in df.columns:
            df["_origin"] = [normalize_origin(str(o), str(s)) for o, s in zip(df["Origin"], site_col)]
        else:
            df["_origin"] = site_col.apply(lambda s: normalize_origin("", s))
...
def build_section(df25, df26):
    combined = pd.concat([df25, df26], ignore_index=True) if not df25.empty or not df26.empty else pd.DataFrame()
    if combined.empty:
    ...
    # Aggregate to flat rows: (year, month, owner, origin, pulau, area)
...
def main():
    ...
    for section, years in SHEET_CONFIG.items():
        ...
        for yr_str, cfg in years.items():
            ...
            if not url:
                print(f"  ⚠️  {cfg['env_key']} missing, skip.")
        ...
        print(f"  ✅ {section}: {len(output[section]['rows'])} agg rows")
